...
func get_commands() -> Dictionary:
	return {
		...
		"get_project_memory": _get_project_memory,
		"set_project_memory": _set_project_memory,
		"remove_project_memory": _remove_project_memory,
		"clear_project_memory": _clear_project_memory,
	}
...
const MCP_MEMORY_PATH := "user://mcp_project_memory.json"


func _load_project_memory() -> Dictionary:
	if not FileAccess.file_exists(MCP_MEMORY_PATH):
		return {"facts": [], "updated_at": ""}
	var file := FileAccess.open(MCP_MEMORY_PATH, FileAccess.READ)
	if file == null:
		return {"facts": [], "updated_at": ""}
...
func _save_project_memory(data: Dictionary) -> bool:
	data["updated_at"] = Time.get_datetime_string_from_system()
	var file := FileAccess.open(MCP_MEMORY_PATH, FileAccess.WRITE)
	if file == null:
		return false
...
func _get_project_memory(_params: Dictionary) -> Dictionary:
	var data := _load_project_memory()
	var facts: Array = data.get("facts", [])
	return success({
		"facts": facts,
		"count": facts.size(),
		"path": MCP_MEMORY_PATH,
		"plain_english": "%d project fact(s) saved for the next chat session." % facts.size(),
	})


func _set_project_memory(params: Dictionary) -> Dictionary:
	var result := require_string(params, "fact")
	if result[1] != null:
		return result[1]
	var fact: String = result[0].strip_edges()
	var data := _load_project_memory()
	var facts: Array = data.get("facts", [])
	if fact not in facts:
		facts.append(fact)
	data["facts"] = facts
	if not _save_project_memory(data):
		return error_internal("Could not write %s" % MCP_MEMORY_PATH)
	return success({
		"added": fact,
		"count": facts.size(),
		"path": MCP_MEMORY_PATH,
		"plain_english": "Saved project fact (%d total). Call get_project_memory after chat resets." % facts.size(),
	})


func _remove_project_memory(params: Dictionary) -> Dictionary:
	var result := require_string(params, "fact")
	if result[1] != null:
		return result[1]
	var fact: String = result[0]
	var data := _load_project_memory()
	var facts: Array = data.get("facts", [])
	var idx := facts.find(fact)
	if idx < 0:
		return error_not_found("Fact '%s'" % fact)
	facts.remove_at(idx)
	data["facts"] = facts
	_save_project_memory(data)
	return success({"removed": fact, "count": facts.size()})


func _clear_project_memory(_params: Dictionary) -> Dictionary:
	_save_project_memory({"facts": []})
	return success({
		"cleared": true,
		"plain_english": "All project memory facts cleared.",
	})
