...
func get_commands() -> Dictionary:
	return {
		...
		"get_mcp_activity_log": _get_mcp_activity_log,
		"clear_mcp_activity_log": _clear_mcp_activity_log,
		"save_mcp_activity_log": _save_mcp_activity_log,
		"get_class_doc": _get_class_doc,
	}
...
func _get_class_doc(params: Dictionary) -> Dictionary:
	var result := require_string(params, "class_name")
	if result[1] != null:
		return result[1]
	var class_name: String = result[0].strip_edges()
	var include_inherited: bool = optional_bool(params, "include_inherited", true)
	var max_methods: int = optional_int(params, "max_methods", 40)
	var max_properties: int = optional_int(params, "max_properties", 30)

	if not ClassDB.class_exists(class_name):
		var matches: Array[String] = []
		for cn: String in ClassDB.get_class_list():
			if class_name.to_lower() in cn.to_lower():
				matches.append(cn)
				if matches.size() >= 15:
					break
		return error_not_found(
			"Class '%s'" % class_name,
			"Try an exact Godot 4 class name. Similar: %s" % ", ".join(matches) if matches.size() > 0 else "Use list_scripts or node types from get_scene_tree.",
		)

	var parent := ClassDB.get_parent_class(class_name)
	var methods: Array = []
	for method_info in ClassDB.class_get_method_list(class_name, include_inherited):
		if method_info is Dictionary:
			var m: Dictionary = method_info
			methods.append({
				"name": m.get("name", ""),
				"return_type": m.get("return", ""),
				"arg_count": (m.get("args", []) as Array).size() if m.get("args") is Array else 0,
			})
		if methods.size() >= max_methods:
			break

	var properties: Array = []
	for prop_info in ClassDB.class_get_property_list(class_name, include_inherited):
		if prop_info is Dictionary:
			var p: Dictionary = prop_info
	...
	var signals: Array = []
	for sig_info in ClassDB.class_get_signal_list(class_name, include_inherited):
		if sig_info is Dictionary:
			signals.append({
	...
	var doc_text := ""
	if ClassDB.has_method("class_get_doc"):
		doc_text = ClassDB.class_get_doc(class_name)

	var plain := "%s extends %s. %d method(s), %d editor property(ies), %d signal(s)." % [
		class_name,
		parent if not parent.is_empty() else "(none)",
		methods.size(),
		properties.size(),
		signals.size(),
	]
	if not doc_text.is_empty():
		plain += " Doc: %s" % doc_text.substr(0, 200).replace("\n", " ")
	...
	return success({
		...
		"signals": signals,
		"methods_truncated": methods.size() >= max_methods,
		"plain_english": plain,
	})


func _activity_log() -> Node:
	if editor_plugin and editor_plugin.get("activity_log"):
		return editor_plugin.activity_log
	return null


func _get_mcp_activity_log(params: Dictionary) -> Dictionary:
	var log := _activity_log()
	if log == null:
		return error_internal("MCP activity log is not available (plugin not fully loaded)")

	var max_entries: int = optional_int(params, "max_entries", 50)
	var errors_only: bool = optional_bool(params, "errors_only", false)
	var since_index: int = optional_int(params, "since_index", -1)
	var as_text: bool = optional_bool(params, "as_text", false)
	var include_params: bool = optional_bool(params, "include_params", true)
	var include_responses: bool = optional_bool(params, "include_responses", true)
	var full_responses: bool = optional_bool(params, "full_responses", true)

	var entries: Array = log.get_entries(max_entries, errors_only, since_index)
	if as_text:
		return success({
			"text": log.export_text(include_params, include_responses, full_responses, errors_only, max_entries),
			"entry_count": log.get_entry_count(),
		})
...
func _clear_mcp_activity_log(_params: Dictionary) -> Dictionary:
	var log := _activity_log()
	if log == null:
		return error_internal("MCP activity log is not available")
	log.clear()
	return success({"cleared": true})


func _save_mcp_activity_log(params: Dictionary) -> Dictionary:
	var log := _activity_log()
	if log == null:
		return error_internal("MCP activity log is not available")

	var path: String = optional_string(params, "path", "user://mcp_activity_log.txt")
	var include_params: bool = optional_bool(params, "include_params", true)
	var include_responses: bool = optional_bool(params, "include_responses", true)
	var full_responses: bool = optional_bool(params, "full_responses", true)
	var errors_only: bool = optional_bool(params, "errors_only", false)

	var result: Dictionary = log.save_to_file(path, include_params, include_responses, full_responses, errors_only)
	if not result.get("saved", false):
		return error_internal("Could not save activity log: %s" % result.get("error", "unknown"))
